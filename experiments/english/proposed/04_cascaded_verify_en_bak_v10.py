...
ENGLISH_SLANG_KB = """
GROUP 1: HATE (TARGETED DEHUMANIZATION & AGGRESSIVE SLURS)
- PROTECTED GROUPS: Attacks on Race, Religion, sexuality, etc.
...
- GENERALIZATIONS: "illegal immigrants are criminals", "moslem terrorists", "jewish profit from war".
- IDEOLOGY: Promoting "White Supremacy", "KKK", "Nazi".

GROUP 2: OFFENSIVE (PERSONAL INSULTS & TOXICITY)
- PERSONAL ATTACKS: "retard", "idiot", "stupid", "pos", "asshole", "trash", "clown", "dumbass", "bitch", "hoe", "cunt", "failure".
- AGGRESSIVE: "stfu", "fucking hate you", "get out", "piece of shit".
- NON-TARGETED: Toxic language not targeting an identity group.

GROUP 3: NORMAL (SAFE & DISCUSSIONS)
- CRITICISM: Criticizing ideas/politics without personal insults.
- IDENTITY: Neutral mention of identity groups.
...
2. RATIONALE EXTRACTION (CRITICAL FOR SPAN IoU): 
   - Extract the FULL EVIDENCE that makes the text toxic (Full Logical Clause).
   - If the attack is "these people are a virus to our nation", extract the whole clause, NOT just "virus".
   - Include the target group name if it's part of the attacking phrase.
    ...
    def process_batch(self, texts):
        ...
        if to_verify_indices:
            ...
            for k, idx in enumerate(to_verify_indices):
                ...
                # Phase 11.2: Rationale Fusion - EXCLUSIVE JUDGE for Max Precision (Target 0.48)
                if cleaned_spans:
