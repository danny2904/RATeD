...
ENGLISH_SLANG_KB = """
GROUP 1: HATE (TARGETED DEHUMANIZATION & VIOLENCE)
- SLURS (Primary): "nigger", "kike", "paki", "coon", "spic", "beaner", "chink", "tranny", "muzzie".
- DEHUMANIZATION: Comparing groups to "vermin", "cockroaches", "animals", "shit", "parasites", "virus". 
- EXTREME METAPHORS: "dating monkeys", "limitless parasites", "plantation slaves".
- IDEOLOGY: "White Supremacy", "KKK", "White Sharia", "Nazi/NSDAP" identification.

GROUP 2: OFFENSIVE (GENERAL TOXICITY, INSULTS & NEGATIVE TOPICS)
- TOXIC TOPICS: "moslem terrorist", "illegal immigrants", "black violence", "jewish profit" (often Gold OFFENSIVE).
- INSULT CLUSTERS: "fucking retarded", "total idiot", "stupid pos", "ghetto mess", "useless trash", "brainless clown", "dumbass bitch".
- AGGRESSIVE: "stfu and die", "super fucking hate you", "get the fuck out", "piece of shit".
- DEROGATORY STEREOTYPES: "ghetto", "plantation", "hood", "ratchet" used as insults.
- NON-TARGETED SLURS: "hoe", "pussy", "bitch", "cunt" used as general insults.

GROUP 3: NORMAL (SAFE & DESCRIPTIVE)
- RECLAIMED: "nigga" or "niggas" in casual AAE context (e.g. "my nigga").
- DESCRIPTIVE: Using "immigrants", "muslims", "black", "gay" for news, facts, or neutral descriptions.
- EMPHASIS: Profanity used for excitement ("fucking love this", "holy shit"). 
- CITATION: Asking about or discussing a slur word itself.
"""
...
3. DEFINITIONS:
   - HATE: Targeted abuse, dehumanization, or threats against protected groups (race, religion, gender, etc.).
   - OFFENSIVE: Profanity used as insult, non-targeted insults, or generally toxic/aggressive language.
   - NORMAL: Safe content, neutral discussions, or swearing used ONLY for emphasis without insulting anyone.
    ...
    def process_batch(self, texts):
        ...
        if to_verify_indices:
            ...
            for k, idx in enumerate(to_verify_indices):
                ...
                # Phase 11.2: Rationale Fusion - EXCLUSIVE JUDGE strategy (Higher Precision for 0.48 target)
                if cleaned_spans:
                    token_mask_for_group3 = build_span_token_mask(text_for_idx, cleaned_spans, self.rated.tokenizer, max_len=128)
                else:
                    # Fallback to backbone only if judge says toxic but provides no spans
                    token_mask_for_group3 = rated_results[idx].get('token_mask') or [0]*128
