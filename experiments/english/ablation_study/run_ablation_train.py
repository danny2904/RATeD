...
# --- STRICT IMPORTS FROM COMMON MODULES ---
try:
...
class HateXplainMultiTaskModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # Token classification (BIO tagging logic implicitly learned or plain binary)
        token_logits = self.token_classifier(self.dropout(sequence_output))
        
        if self.use_fusion:
            # simple attention-based fusion
            token_probs = torch.softmax(token_logits, dim=-1)
            toxic_probs = token_probs[:, :, 1].unsqueeze(-1) # Probability of being toxic rationale
...
class HateXplainDataset(Dataset):
    def __init__(self, data_path, tokenizer, max_len=128, split='train'):
        ...
        self.label_map = {"hatespeech": 0, "normal": 1, "offensive": 2} # Align with project standard
    ...
    def __getitem__(self, idx):
        ...
        # Merge logic (Union of all annotators) as decided
        ...
        return {
            ...
            "class_labels": torch.tensor(label_id, dtype=torch.long), 
            "token_labels": token_labels,
            "offsets": offsets, # Return offsets tensor
...
def train(args, model, tokenizer, device):
    ...
    val_dataset = HateXplainDataset(args.data_path, tokenizer, split='val') # or 'test' if no val split
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.epochs}"):
            ...
            # Helper for token loss: flatten
            active_loss = attention_mask.view(-1) == 1
            active_logits = tok_logits.view(-1, 2)
            active_labels = torch.where(
                active_loss, tok_labels.view(-1), torch.tensor(tok_criterion.ignore_index).type_as(tok_labels)
            )
...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            all_tok_preds.append(torch.argmax(tok_logits, 2).cpu())
    ...
    return {
        "cls_f1": cls_f1, "cls_acc": cls_acc, "cls_precision": cls_p, "cls_recall": cls_r, "cls_cm": cls_cm,
        "span_f1": span_f1,
        "token_acc": tok_metrics['acc'],
        "token_mf1": tok_metrics['f1'],
        "token_f1_pos": tok_metrics['f1_pos'],
        "token_auprc": tok_metrics['auprc'] if tok_metrics['auprc'] is not None else 0.0,
        "token_precision": tok_metrics['precision'],
        "token_recall": tok_metrics['recall']
    }
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="experiments/english/ablation_study/no_fusion") # Default to new dir
    ...
    parser.add_argument("--use_fusion", type=int, default=1, help="Use Explanation Fusion (1/0)") # NEW FLAG
    ...
    if args.do_train:
        print(f"Initializing model: {args.base_model} (Fusion: {args.use_fusion}, Consistency: {args.use_consistency})")
        tokenizer = AutoTokenizer.from_pretrained(args.base_model)
        model = HateXplainMultiTaskModel(args.base_model, use_fusion=(args.use_fusion==1))
        model.to(device)
        
        train(args, model, tokenizer, device)
        
        # After training, load best model for eval
        ...
        try:
            ...
            model = HateXplainMultiTaskModel(base_name, use_fusion=(args.use_fusion==1))
            ...
            if os.path.exists(ckpt_path):
                ...
                model.load_state_dict(torch.load(ckpt_path, map_location=device))
                 ...
                 if os.path.exists(bin_path):
                      ...
                      print("⚠️ No specific checkpoint found. Inference might be random.")
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Testing"):
            ...
            # Faithfulness Data Prep
            batch_offsets = batch['offsets'].numpy()
            batch_texts = batch['text']
            
            for i in range(len(batch_texts)):
                # Reconstruct spans from token preds
    ...
    span_metrics = {
       "token_acc": tok_metrics_res['acc'],
       "token_p": tok_metrics_res['precision'],
       "token_r": tok_metrics_res['recall'],
       "token_f1": tok_metrics_res['f1'],
       "token_f1_pos": tok_metrics_res['f1_pos'],
       "token_auprc": tok_metrics_res.get('auprc', 0.0),
       "span_f1": span_f1
    }
    
    # 3. AUROC & Bias
    bin_gold = [1 if l != 1 else 0 for l in all_labels] # 1 is Normal, so others are 1(Toxic)
    auroc = calculate_auroc(bin_gold, toxic_probs_list)
    
    bias_metrics = {}
    bias_items = load_bias_metadata_from_prepared(args.data_path, len(all_labels))
    if bias_items:
        bias_metrics = calculate_bias_metrics(bias_items, bin_gold, toxic_probs_list)
    ...
    if 'calculate_faithfulness_metrics' in globals():
         try:
             faithfulness = calculate_faithfulness_metrics(pipeline, test_dataset, faith_results)
         except Exception as e:
    ...
    # --- REPORTING ---
    # Log filename dynamic based on ablation
    variant_name = "Full"
    if args.use_fusion == 0: variant_name = "NoFusion"
    if args.use_consistency == 0: variant_name = "NoConsistency"
    
    extra_info = "-" * 50 + "\n"
    title_report = f"RATeD Ablation ({variant_name})"
    
    extra_info += ">> GROUP 4: FAITHFULNESS & FAIRNESS\n"
    extra_info += "-" * 50 + "\n"
    extra_info += f"{'Faithful. Comp (High is good)':<30} | {faithfulness.get('faithfulness_comprehensiveness', 0):.4f}\n"
    extra_info += f"{'Faithful. Suff (Low is good)':<30} | {faithfulness.get('faithfulness_sufficiency', 0):.4f}\n"
    extra_info += "-" * 45 + "\n"
    ...
    log_content = generate_report_string(title_report, cls_metrics, span_metrics, cls_cm)
    log_content += "\n" + extra_info
    
    print(log_content)
    
    log_name = os.path.join(args.output_dir, f"ablation_metrics_{variant_name}.log")
